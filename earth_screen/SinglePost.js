...
import { SafeAreaView, TouchableOpacity, View, Text, ScrollView, Image, TextInput, Dimensions, KeyboardAvoidingView } from 'react-native';
...
export default function SinglePost({ navigation: { goBack } }) {
    ...
    useEffect(() => {
        ...
        fetch(api, { method: 'GET', headers: { 'Content-Type': 'application/json' } })
            .then((res) => res.json())
            .then((resJson) => { setData(resJson); setCmt(resJson.comment) })
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
            <View style={{ flex: 1, backgroundColor: '#fff' }}>
                ...
                <View style={{ flexDirection: "row", padding: 7, borderTopWidth: 1, borderColor: '#f2f2f2' }}>
                    ...
                    <TextInput placeholder="Viết bình luận..." style={{ fontSize: 20, marginHorizontal: 10, width: '75%', backgroundColor: '#f2f2f2', borderRadius: 5, padding: 12, }} onChangeText={(text) => setShow(text)}></TextInput>
                    {show ? (
                        <TouchableOpacity style={{ marginLeft: 5, justifyContent: "center", alignItems: 'center' }}>
